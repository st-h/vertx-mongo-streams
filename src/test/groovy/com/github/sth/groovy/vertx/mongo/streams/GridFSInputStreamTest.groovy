...
import com.github.sth.groovy.vertx.mongo.streams.util.DrainHandler;
...
public class GridFSInputStreamTest {
  ...
  public void happyPathReadAfterWrite() {
    ...
    Buffer buffer1 = new Buffer(io.vertx.core.buffer.Buffer.buffer(ByteUtil.randomBytes(2048)));
    Buffer buffer2 = new Buffer(io.vertx.core.buffer.Buffer.buffer(ByteUtil.randomBytes(2048)));
  ...
  public void testWriteQueueFull() {
    ...
    Buffer buffer = new Buffer(io.vertx.core.buffer.Buffer.buffer(ByteUtil.randomBytes(4096)));
    ...
    buffer = new Buffer(io.vertx.core.buffer.Buffer.buffer(ByteUtil.randomBytes(4095)));
    inputStream.write(buffer);
    Assert.assertFalse(inputStream.writeQueueFull());

    // queue should be full (8192 bytes in queue)
    buffer = new Buffer(io.vertx.core.buffer.Buffer.buffer(ByteUtil.randomBytes(1)));
    inputStream.write(buffer);
  ...
  public void testDrainHandler() {
    ...
    Buffer buffer = new Buffer(io.vertx.core.buffer.Buffer.buffer(ByteUtil.randomBytes(8192)));
  ...
  public void testSetWriteQueue() {
    ...
    Buffer buffer = new Buffer(io.vertx.core.buffer.Buffer.buffer(ByteUtil.randomBytes(1)));
    inputStream.write(buffer);
